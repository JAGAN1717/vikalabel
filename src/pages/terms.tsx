...
export default function TermsPage() {
  ...
  useEffect(() => {
    const find = data?.data?.find((e: { title: string; }) => e.title == "terms_of_service")
    if (find) {
      setTerms(JSON.parse(find?.value))
    }
  }, [data])


  // console.log('skdshkdsdsd', terms)
              ...
              <div className="md:w-9/12 md:pb-10 ltr:md:pl-8 rtl:md:pr-8">

                {
                  terms?.description ?
                    <Element
                      key={terms?.title}
                      name={makeTitleToDOMId(terms?.title)}
                      className="mb-10"
                    >
