...
export default function ReturnpolicyPage() {
  ...
  const [returnPolicy, seteturnPolicy] = useState<any>()
  ...
  useEffect(() => {
    const find = data?.data?.find((e: { title: string; }) => e.title == "refund_policy")
    if (find) {
        seteturnPolicy(JSON.parse(find?.value))
    }
  }, [data])
              ...
              <div className="md:w-9/12 md:pb-10 ltr:md:pl-8 rtl:md:pr-8">

                {
                  returnPolicy?.description ?
                    <Element
                      key={returnPolicy?.title}
                      name={makeTitleToDOMId(returnPolicy?.title)}
                      className="mb-10"
                    >
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: t(returnPolicy?.description) }}
                      />
                    </Element> :
                    <NotFound
                      text="Return Policy Not yet"
